...
function replaceRule(value, stage) {
  ...
  if (
    ...
    /babel-preset-gatsby[/\\]dependencies\.js/.test(value.use[0].options.presets)
  ) {
    return null;
  }
...
exports.onCreateDevServer = ({ app, store }, { publicPath = `admin` }) => {
  ...
  app.get(`/${publicPathClean}`, function(req, res) {
    res.sendFile(path.join(program.directory, `public`, publicPathClean, `index.html`), err => {
      if (err) {
...
exports.onCreateWebpackConfig = (
  ...
  const config = {
    ...gatsbyConfig,
    entry: {
      cms: [path.join(__dirname, `cms.js`), enableIdentityWidget && path.join(__dirname, `cms-identity.js`)]
        .concat(modulePath)
        .filter(p => p)
    },
    output: {
    ...
    module: {
      rules: deepMap(gatsbyConfig.module.rules, value => replaceRule(value, stage)).filter(Boolean)
    },
    plugins: [
      ...
      ...gatsbyConfig.plugins.filter(
        plugin =>
          ![`MiniCssExtractPlugin`, `GatsbyWebpackStatsExtractor`].find(
            pluginName => plugin.constructor && plugin.constructor.name === pluginName
          )
      ),
